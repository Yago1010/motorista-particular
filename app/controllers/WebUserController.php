...
class WebUserController extends \BaseController {
	...
	public function userRequestTrip()
	{
		...
		{	
			...
			if($walker_detail){$walker_detail=$walker_detail;}else{$walker_detail='';}
	...
	public function request_fare(){
        ...
        if(Input::has('provider')){
        	 ...
        	 $price_per_unit_distance = $pt->price_per_unit_distance*$distanceNew;
    	...
    	if($promo_code){
			...
			if($promosett->value==1){
				if($request->promo_code!=NULL){
					...
					if($promo_type==1){
						$discount = $reqserv_total*$promo_value/100;
					}elseif($promo_type==2){
	...
	public function userSave()
	{
		...
		if ($validator->fails()) {
			...
			if(Owner::where('email',$email)->count() == 0){
				...
				$owner->dog_id = 0;
				$owner->device_token = "";
				$owner->device_type = "android";
				$owner->login_by = "manual";
				$owner->social_unique_id = "";
				$owner->latitude = 0;
				$owner->longitude = 0;
				$owner->debt = 0;
				...
				// Referral is accepted without verification.
				if($referral_code != ""){
					$owner->referred_by = 0;
				}
				$owner->save();
			...
			if($_GET){
	        	...
	        	$distance = $settings->value;
						if($typestring==""){
						          ...
						          $query = "SELECT walker.id,walker.first_name,walker.last_name,walker.latitude,walker.longitude, 1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(walker.latitude) ) * cos( radians(walker.longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(walker.latitude) ) ) as distance from walker JOIN walker_services where walker.is_available = 1 and walker.is_active = 1 and walker.is_approved = 1 and (1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(walker.latitude) ) * cos( radians(walker.longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(walker.latitude) ) ) ) <= $distance and walker.id = walker_services.provider_id and walker_services.type = $typestring order by distance";
						    }
			    $walkers = DB::select(DB::raw($query));
