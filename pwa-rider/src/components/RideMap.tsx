...
function FitBounds({ a, b }: { a: Geo; b: Geo }) {
  const map = useMap()
  useEffect(() => {
    map.fitBounds(L.latLngBounds(L.latLng(a.lat, a.lng), L.latLng(b.lat, b.lng)), {
      padding: [72, 72],
      maxZoom: 16,
      animate: true,
    })
...
export function RideMap({ origin, dest, className, routePath }: RideMapProps) {
    ...
    <div className={className ?? 'pwa-map-inner'}>
      <MapContainer center={center} zoom={zoom} scrollWheelZoom className="pwa-leaflet-map" zoomControl={false}>
        <TileLayer
          ...
          subdomains="abcd"
        />
        {origin && dest && routePath && routePath.length > 2 ? (
          <FitRoute path={routePath} a={origin} b={dest} />
        ) : origin && dest ? (
          <FitBounds a={origin} b={dest} />
        ) : (
